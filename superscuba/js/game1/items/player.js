...
export default class Player extends Item {
  ...
  add() {
    ...
    // set item image/
    const img_path = "/superscuba/media/game1_assets/player.gif";
  ...
  move(dt, input) {
    ...
    if ((!input["ArrowRight"] && !input["ArrowLeft"]) || (input["ArrowRight"] && input["ArrowLeft"])) {
      this.vel_x = this.calcFriction(this.vel_x, friction);
    }

    if ((!input["ArrowDown"] && !input["ArrowUp"]) || (input["ArrowDown"] && input["ArrowUp"])) {
    ...
    if (this.x < this.map_w * player_margin_x) {
      ...
      this.vel_x = 0;
    } else if (this.x + this.width > this.map_w - this.map_w * player_margin_x) {
    ...
    switch (perk.name) {
      case "speed_boost":
        if (typeof this.perks["speed_boost"] === "undefined") {
          ...
          if (this.perks["speed_boost"] + cfg.speed_boost_duration - pickup_time > cfg.speed_boost_max) {
      ...
      case "magnet":
        if (typeof this.perks["magnet"] === "undefined") {
          ...
          if (this.perks["magnet"] + cfg.magnet_duration - pickup_time > cfg.magnet_max) {
      ...
      case "gold_magnet":
        if (typeof this.perks["gold_magnet"] === "undefined") {
          ...
          if (this.perks["gold_magnet"] + cfg.gold_magnet_duration - pickup_time > cfg.gold_magnet_max) {
